...
# Função para detectar uma cor específica na imagem
def detect_color(frame, lower_bound, upper_bound, color_name):
    ...
    # Define a faixa de cores que queremos detectar
    ...
    if max_contour is not None:
        # Desenha um retângulo ao redor do maior objeto encontrado
        x, y, w, h = cv2.boundingRect(max_contour)
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv2.putText(frame, color_name, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

# Inicialize a câmera (0 é o índice da câmera padrão)
...
while True:
    ...
    # Execute o OCR (Reconhecimento Óptico de Caracteres) na imagem
    text = pytesseract.image_to_string(erode, config='--psm 6')

    # Verifique se as letras 'r', 'g' e 'b' estão presentes no texto reconhecido
    if 'r' in text.lower():
...
# Libere a câmera e feche a janela da câmera
cap.release()
